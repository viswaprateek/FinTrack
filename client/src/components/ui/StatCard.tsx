    ...
    <Card className={cn(compact ? 'p-4' : 'p-5', 'h-full')}>
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <p className={cn('font-medium uppercase tracking-wide text-muted', compact ? 'text-[10px]' : 'text-xs')}>
            {label}
          </p>
          ...
          {subLabel && (
            <p className={cn('text-muted', compact ? 'mt-0.5 text-[10px] line-clamp-2' : 'mt-1 text-xs')}>
              {subLabel}
            </p>
          )}
        </div>
